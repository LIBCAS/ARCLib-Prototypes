...
import cz.inqool.uas.transformer.Transformer;
...
public class FileRepository {
    ...
    public FileRef create(InputStream stream, String id, String contentType, boolean indexContent) {
        ...
        checked(() -> {
            ...
            if (indexContent) {
                if (transformer.support(contentType, indexContentType)) {
                    try (InputStream transformIn = newInputStream(path)) {
                        ByteArrayOutputStream transformOut = new ByteArrayOutputStream();
                        transformer.transform(contentType, indexContentType, transformIn, transformOut);

                        ref.setIndexedContent(true);
                        ref.setContent(new String(transformOut.toByteArray(), Charset.forName("UTF-8")));
                    }
                } else {
                    log.warn("Trying to index unsupported content type '{}' of {}.", contentType, ref.getId());
                }
            }
        });
